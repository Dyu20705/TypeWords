<script setup lang="ts">
import { APP_NAME, GITHUB } from '@/core/config/env.ts'
import ChannelIcons from '@/components/channel-icons/ChannelIcons.vue'
</script>

<template>
  <h1>{{ APP_NAME }}</h1>
  <div class="w-120">
    <p class="text-xl">
      {{ $t('about_thanks') }}
    </p>
    <p>
      {{ $t('github_address') }}<a :href="GITHUB" target="_blank">{{ GITHUB }}</a>
    </p>
    <div class="flex flex-col gap-2 mt-20">
      <div>{{ $t('help') }}: <RouterLink to="/help">{{ $t('help') }}</RouterLink></div>
      <div>
        {{ $t('about_ticket_feedback')
        }}<a :href="`${GITHUB}/issues`" target="_blank">{{ GITHUB }}/issues</a>
      </div>
      <div class="flex items-center">
        {{ $t('about_other_channels') }}
        <ChannelIcons type="horizontal" :share="false" :github="false" />
      </div>
    </div>
  </div>
</template>

<style scoped lang="scss"></style>
