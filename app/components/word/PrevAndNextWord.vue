...
<template>
  ...
  >
    <Tooltip :title="`${$t('shortcut_previous')}(${settingStore.shortcutKeyMap[ShortcutKey.Previous]})`">
    ...
    <div class="center gap-1 absolute w-full cp" v-if="settingStore.showConflictNotice2" @click="emit('openNotice')">
      ...
      <span class="">{{ $t('cannot_input_hint') }}</span>
    </div>

    <Tooltip :title="`${$t('shortcut_next')}(${settingStore.shortcutKeyMap[ShortcutKey.Next]})`">
