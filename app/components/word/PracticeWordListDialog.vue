...
<template>
  <Dialog v-model="model" padding :title="$t('task')">
    <div class="pb-4 h-80vh flex gap-4">
      <div class="h-full flex flex-col gap-2">
        <div class="flex justify-between items-center">
          <span class="title">{{ $t('new_words_count', { count: data.new.length }) }}</span>
          <Checkbox v-model="showTranslate">{{ $t('translation') }}</Checkbox>
        </div>
        <BaseTable
      ...
      <div class="h-full flex flex-col gap-2" v-if="data.review.length">
        <div class="flex justify-between items-center">
          <span class="title">{{ $t('review_words_count', { count: data.review.length }) }}</span>
        </div>
        <BaseTable
          class="overflow-auto flex-1 w-85"
          :list="data.review"
